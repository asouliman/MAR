...
function FlyingVehicle(configuration) {
	...
	if (!configuration.hasOwnProperty('mass')) {
		configuration.mass = 50;
	}
	if (!configuration.hasOwnProperty('xLength')) {
		configuration.xLength = 5;
	}
	if (!configuration.hasOwnProperty('yLength')) {
		configuration.yLength = 2;
	}
	if (!configuration.hasOwnProperty('zLength')) {
		configuration.zLength = 2;
	}
	...
	this.update = function (dt) {
		...
		var currentForce = this.force.clone();
		this.acceleration = currentForce.multiplyScalar(1.0/this.mass);
